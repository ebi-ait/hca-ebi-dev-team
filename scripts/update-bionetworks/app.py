...
class Project:
    ...
    def with_schema_version(self, version: str):
        self.content['describedBy'] = f'https://schema.data.humancellatlas.org/type/project/{version}/project'
        return self
...
def update_bionetwork_for_project(project_uuid: str,
    ...
    add_bionetwork(project, bionetwork, update_op)
    if validate:
    ...
    logging.info(f'project {project_uuid} modified. {update_op} with network {bionetwork}. ')


def add_bionetwork(project, bionetwork: HCABionetwork, update_op):
    ...
    elif update_op == 'replace':
        hca_bionetworks.clear()
    else:
        raise ValueError(f'update_op {update_op} not supported. Only use "add" or "replace')
    if new_element:
...
def run():
    ...
    project_uuids = [
        'e4b2e4d9-2b9b-46cf-b0e0-bb23ff28030a',
    ]
